...
namespace {
size_t activation_write_callback(void* contents, size_t size, size_t nmemb, void* userp) {
...
ActivationService::activate(const ActivateRequest& request) {
    ...
    curl_easy_setopt(pimpl_->curl, CURLOPT_WRITEFUNCTION, activation_write_callback);
